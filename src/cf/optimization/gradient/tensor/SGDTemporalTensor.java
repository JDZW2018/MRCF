package cf.optimization.gradient.tensor;
...
import cf.optimization.gradient.common.CombinerReducer;
 ...
 *
 */
public class SGDTemporalTensor implements TensorSolver {
	
	/**
	 * Loss function for tensor
	 */
	protected static TemporalLoss lossFunc = null;
	/**
	...
	public void initialize(Loss lossFunc, double lambda,
		...
		SGDTemporalTensor.lossFunc = (TemporalLoss) lossFunc;
		SGDTemporalTensor.lambda = lambda;
		SGDTemporalTensor.learningRate = learningRate;
		SGDTemporalTensor.numD = numD;
	}
	...
	public void update(DenseDoubleMatrix2D U, DenseDoubleMatrix2D V,
		...
		conf.set("model.loss", ModelOperator.loss2String(conf, SGDTemporalTensor.lossFunc));
		...
		job.setJarByClass(SGDTemporalTensor.class);
		...
		extractUVS(SGDTemporalTensor.U, SGDTemporalTensor.V, SGDTemporalTensor.S, job.getConfiguration(), outputPath);
	...
	public class SGDMapper extends Mapper<LongWritable, Text, Text, Text>{
		...
		public void map(LongWritable key, Text value, Context context)
			...
			DoubleMatrix1D s1 = null;
			if(o==0) s1 = new DenseDoubleMatrix1D(numD);
			else s1 = S.viewRow(o-1);
			DoubleMatrix2D g = lossFunc.getGradient(U.viewRow(p), V.viewRow(q), s1, S.viewRow(o), r, lambda);
			...
			outKey.set("U,"+p);
			outValue.set(StringUtils.array2String(g.viewRow(0).toArray()));
			context.write(outKey, outValue);
			outKey.set("V,"+q);
			outValue.set(StringUtils.array2String(g.viewRow(1).toArray()));
			context.write(outKey, outValue);
			outKey.set("S,"+q);
			outValue.set(StringUtils.array2String(g.viewRow(2).toArray()));
			context.write(outKey, outValue);
		}
		...
		protected void setup(Context context) throws IOException, InterruptedException{
			...
			SGDTemporalTensor.numD = conf.getInt("model.numD", 10);
			SGDTemporalTensor.lambda = conf.getFloat("model.lambda", 0.005f);
			SGDTemporalTensor.lossFunc = (TemporalLoss) ModelOperator.string2Loss(conf,conf.get("model.loss"));
		}
	}
