...
import cf.optimization.gradient.hybrid.DistributedSGDHybrid;
import cf.optimization.gradient.hybrid.SquareHybridLoss;
 ...
 *
 */
public class DistributedHybridMF extends AbstractHybridMF {
	/**
	 * Regression matrix
	 */
	 ...
	 * Optimization method
	 */
	protected DistributedSGDHybrid solver = null;
	...
	public void initModel(int m, int n, int d, int fm, int fn, String solverName, double lambda, double learningRate, int numIt, String inputPath) throws Exception{
		...
		solver = (DistributedSGDHybrid) Class.forName(solverName).newInstance();
		init(m, d);
	...
	public void buildModel() throws Exception {
		...
		for(int i=0;i<super.numIt;i++){
			...
			solver.update(null, super.V, this.B, super.inputPath, conf.get("hadoop.output.path")+"/DistributedHybridMF/"+i+"/");
		}			
	}
	 ...
	 */
	protected void init(int m, int d) throws IOException, InterruptedException, ClassNotFoundException, URISyntaxException{
	...
	public void predictPair(String inputPath, int numD) throws IOException, URISyntaxException, InterruptedException, ClassNotFoundException{
		...
		job.setJarByClass(DistributedHybridMF.class);
		job.setJobName("DistRLMF-Prediction");		
		...
		FileOutputFormat.setOutputPath(job, new Path(conf.get("hadoop.output.path")+"/prediction-DistHybridMF/"));
	...
	public static class PredictionReducer extends Reducer<Text, Text, Text, DoubleWritable>{
		 ...
		 * Loss function
		 */
		private SquareHybridLoss lossFunc = null;
		...
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			while(it.hasNext()){
				...
				if (items[items.length-1].equals("U")){	//U
			...
			for(Integer q : idList){
				double r = lossFunc.getPrediction(U, V.viewRow(q), B, Fu, Fv.viewRow(q));
				outKey.set(key+","+q);
				outValue.set(r);
		...
		protected void setup(Context context) throws IOException, InterruptedException{
			...
			Fv = MatrixIO.readDenseMatrixFromText(paths[1].toString());
			B = MatrixIO.readDenseMatrixFromText(paths[2].toString());
			lossFunc = new SquareHybridLoss();
		}
	}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	@Deprecated
