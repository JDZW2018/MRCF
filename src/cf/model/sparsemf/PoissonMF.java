...
import cf.optimization.gradient.sparse.PoissonLoss;
...
public class PoissonMF extends AbstractMF {
	...
	public void predictPair(String inputPath, int numD) throws Exception {
		Configuration conf = FileOperators.getConfiguration();
		DistributedMFPrediction.predictPair(inputPath, conf.get("hadoop.output.path"), "cf.model.sparsemf.PoissonMF", numD);
	}

	@Override
	public void predictAll() throws Exception {
		Configuration conf = FileOperators.getConfiguration();
		DistributedMFPrediction.predictAll(conf.get("hadoop.output.path"), "cf.model.sparsemf.PoissonMF");			
	}
}
