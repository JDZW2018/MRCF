...
import cf.optimization.gradient.sparse.LogSquareLoss;
import utils.hadoop.FileOperators;


/**
 * Matrix Factorization with Log Square Loss
 * log[(R - UV')^2]
 * @author tigerzhong
 *
 */
public class LogGaussianMF extends AbstractMF{
	...
	public void buildModel() throws Exception {
		LogSquareLoss lossFunc = new LogSquareLoss();
		Configuration conf = FileOperators.getConfiguration();
	...
	public void predictPair(String inputPath, int numD) throws Exception {
		Configuration conf = FileOperators.getConfiguration();
		DistributedMFPrediction.predictPair(inputPath, conf.get("hadoop.output.path"), "cf.model.sparsemf.GaussianMF", numD);
	}

	@Override
	public void predictAll() throws Exception {
		Configuration conf = FileOperators.getConfiguration();
		DistributedMFPrediction.predictAll(conf.get("hadoop.output.path"), "cf.model.sparsemf.GaussianMF");	
	}
}
