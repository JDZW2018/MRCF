...
import cf.combine.HybridBiasMF;
import cf.model.AbstractMF;
import cf.model.collectivemf.AbstractCMF;
import cf.model.common.MFPrediction;
import cf.model.hybridmf.AbstractHybridMF;
import cf.model.knn.KNN;
import cf.model.knn.KNNPredictionAll;
import cf.model.tensor.AbstractTensor;
...
public class CFDriver {
	 ...
	 * @throws Exception
	 */
	public static void performPrediction(String task) throws Exception{
		if(task.equalsIgnoreCase("predictionPair")){
			MFPrediction.predictAll(inputPath, modelName);
		} else if(task.equalsIgnoreCase("predictionAll")){
			if(modelName.indexOf("KNN")>-1){
				KNNPredictionAll.predict(inputPath, numContext==0);
			} else {
	...
	public static void performTraining() throws Exception {
		...
		if(modelName.indexOf("KNN")>-1){						//KNN
			KNN model = new KNN();
			model.initModel(inputPath, numD, opName, lambda, numContext==0, numUser, numItem);
			model.buildModel();
		}else if(modelName.indexOf("combine")>-1){				//combine
			HybridBiasMF model = new HybridBiasMF();
